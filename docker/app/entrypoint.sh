...
if [ "$(id -u)" = "0" ]; then
    ...
    # Runtime caching: only config:cache and route:cache need runtime env vars.
    # event:cache and view:cache are baked into the image at build time.
    if [ "$APP_ENV" = "production" ] || [ "$APP_ENV" = "staging" ]; then
        echo "Running runtime optimizations for $APP_ENV (role: ${CONTAINER_ROLE:-app})..."
